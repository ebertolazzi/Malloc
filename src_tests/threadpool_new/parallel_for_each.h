...
namespace threadpool {
	...
	*/
	...
	for_each(Iterator first, const Last& last, Function&& fun)
	{
	    ...
	    unsigned int tc =
		impl::GenericThreadPoolTmpl::determine_thread_count(thread_count);

	    if (tc <= 1) {
		return singlethreaded::parallel::for_each(first, last, fun);
	    } else {
		impl::ForEachThreadPoolImpl<Iterator, Last, Function
	...
	for_each(Iterator&& first, const Last& last, Function&& fun)
	{
	    ...
	    typedef impl::IntegralIterator<common_type> CommonIterator;
