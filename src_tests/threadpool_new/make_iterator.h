...
namespace threadpool {
   ...
   * remaining.
   */
  template<class Function, class Enable = void> class FunctionPtr;
   ...
   */
  template<class Function>
  class FunctionInputIterator : public std::iterator<std::input_iterator_tag,decltype(std::declval<Function>()())> {
    typedef std::iterator<std::input_iterator_tag,decltype(std::declval<Function>()())> Base;
    /*
  ...
  public:
    ...
    operator*() const {
      ...
      if (v->last) throw std::out_of_range("");
      return v->value.move();
    }

    bool
    operator==( FunctionInputIterator const & x ) const {
      ...
      if (x.v != nullptr && !(x.v->last || x.v->value_valid)) return x.operator==(*this);
   ...
   */
  template<class Function>
  FunctionInputIteratorRange<Function>
  make_function_input_range(Function&& fun) {
    return FunctionInputIteratorRange<Function>(std::forward<Function>(fun));
  }
