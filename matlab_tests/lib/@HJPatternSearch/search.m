...
function stencil_failure = search(self)
  ...
  stencil_failure = self.best_nearby();
  while stencil_failure
    % reduce the scale
  ...
  lambda  = 1;
  max_der = 0;
  while self.m_fun_evaluation_count < self.m_max_fun_evaluation && lambda > 0.1;
